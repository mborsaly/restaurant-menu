...
export default function CartSheet({
  ...
  return (
    <div dir={rtl ? 'rtl' : 'ltr'} style={{ position: 'relative', padding: '4px 16px 20px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 16 }}>
        {cart.map(item => (
          <div key={item.id} style={{ background: 'white', borderRadius: 16, padding: 12, border: '1px solid rgba(45,42,38,0.06)' }}>
            ...
            {/* Row — locked to direction:'ltr'. Both
                blocks use explicit `order` so the
                stepper's internal [−][qty][+] sequence
                never gets mirrored — only the two
                columns swap sides as whole units.
                Same pattern as ItemSheet.jsx's
                quantity row. */}
            <div style={{
              display: 'flex', alignItems: 'flex-start', gap: 10, direction: 'ltr',
            }}>
              ...
              <div style={{
                ...
                <p style={{
                  fontWeight: 700, fontSize: 13, color: '#2D2A26', marginBottom: 6,
                  fontFamily: arabicFont, textAlign: rtl ? 'right' : 'left',
                }}>
                  {getItemName(item)}
                </p>

                {/* Stepper: always − → qty → +, locked
                    to LTR internally, never flips */}
                <div style={{
      ...
      {/* Subtotal / delivery / total — same label-vs-value
          order pattern as ItemSheet's quantity row */}
      <div style={{ background: 'white', borderRadius: 16, padding: 14, border: '1px solid rgba(45,42,38,0.06)', marginBottom: 16 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, marginBottom: 6, direction: 'ltr' }}>
